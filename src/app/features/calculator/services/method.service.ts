...
/**
 * Builds the full Dough method on the clock: the weigh-in per part and
 * every visible step, the milestones dated with the engine's timings
 * (quarter-hour grid, same clock as the Expert preview), down to the bake.
 */
...
export class MethodService {
  build(
    ...
    const startAt = ceilToQuarterHour(now);
    ...
    return {
      ...
      steps: defs.flatMap((def) =>
        def.steps
          ...
          .map((step) => this.toStep(def, step, startAt)),
      ),
      startAt,
  ...
  private toStep(
    ...
    return {
      ...
      ingredients: step.ingredients
        ? toMethodIngredients(step.ingredients)
        : [],
    };
  }
}
